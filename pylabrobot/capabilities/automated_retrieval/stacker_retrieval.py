...
class StackerRetrieval(AutomatedRetrieval):
  ...
  async def downstack(
    self, stack: Union[ResourceStack, int], tray_index: Optional[int] = None
  ) -> Plate:
    """Move the accessible (top) plate of ``stack`` onto loading tray ``tray_index`` and return it.

    ``tray_index=None`` uses the backend's default tray.
    """
    loading_tray = self._require_loading_tray(tray_index)
    stack = self._resolve_stack(stack)
    ...
    await self.backend.downstack(stack, tray_index=tray_index)
    plate.unassign()
  ...
  async def upstack(
    ...
    """Move a plate from loading tray ``tray_index`` onto ``stack`` (defaults to the plate on
    the tray). ``tray_index=None`` uses the backend's default tray."""
    stack = self._resolve_stack(stack)
    if plate is None:
      plate = self._plate_on_loading_tray(tray_index)
    await self.backend.upstack(stack, plate, tray_index=tray_index)
    plate.unassign()
